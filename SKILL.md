...
浏览器工具按以下顺序自动降级，不因 Chrome 插件不可用而停止：

1. 已安装且可连接用户 Chrome，或任务依赖其现有标签页、登录态、扩展时，使用 `chrome:control-chrome`。
2. Chrome 控制出现插件兼容、连接、授权、标签页附着或超时错误时，立即改用 `browser:control-in-app-browser`，在 Codex 内置浏览器中后台打开本地 `localhost`/`127.0.0.1` 页面、操作、验证并截图；不要反复要求用户重装插件。
3. 两种交互式浏览器都不可用，但项目允许本地自动化时，使用已安装的 Playwright/Chromium 等无头浏览器复现同一路径；记录窗口尺寸、URL、输入样例和命令。若仍不可用，才保留补图项并把 `submission_ready=false`。

切换浏览器后从未覆盖功能继续，不重复已合格截图。使用用户提供的脱敏图片、视频或测试文件时，优先实际上传或选择这些样例完成正常路径和结果页截图，并记录样例来源；不得用伪造结果替代系统输出。每张截图都要记录 `capture_tool`，并人工或 OCR 检查软件名称、版本、个人信息、测试账号、密钥、磁盘路径、错误堆栈、浏览器警告和拼接痕迹。允许裁掉无关窗口边框或做隐私遮盖，但不得拼接成不存在的界面，不得用设计稿冒充运行截图。
...
草稿阶段才可设置 `document.allow_missing_images=true`。正式生成必须关闭它，并确保所有可申报功能已覆盖、所有核心功能有经复核截图、所有图题准确、图片清晰、目录和页码可刷新。生成后检查 DOCX 中 `w:color`：不得出现非 `000000` 或主题控制的文本颜色。
